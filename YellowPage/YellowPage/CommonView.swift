...
class CommonView: UIView {
    ...
    convenience init(with models: [ClientItem]) {
        ...
        let headerLabel = UILabel(frame: CGRect(x: 15, y: 0, width: 0, height: 0))
        headerLabel.text = "常用部门"
        headerLabel.font = UIFont.systemFont(ofSize: 15)
        headerLabel.sizeToFit()
        
        collectionView = UICollectionView(frame: CGRect(x: 0, y: headerLabel.frame.size.height+10, width: width, height: 150), collectionViewLayout: layout)
